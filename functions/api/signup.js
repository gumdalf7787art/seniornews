...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const verificationLink=`${new URL(context.request.url).origin}/api/auth/verify?token=${verificationToken}`; context.waitUntil(sendEmail(context.env,normalizedEmail,'[시니어 뉴스] 이메일을 확인해 주세요',`<p><a href="${verificationLink}">이메일 인증하기</a></p>`));
