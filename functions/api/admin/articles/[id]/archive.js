...
export async function onRequestPost(context) {
  ...
  if (auth.user.role === 'editor' && Number(article.author_id) !== Number(auth.user.id)) return json({ message: '본인이 작성한 기사만 비공개로 전환할 수 있습니다.' }, 403);
  if (article.status === 'archived') return json({ success: true, status: 'archived' });

  await context.env.DB.prepare("UPDATE articles SET status='archived', updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(id).run();
  await audit(context.env, auth.user.id, 'archive', 'article', id, { title: article.title, previous_status: article.status });
