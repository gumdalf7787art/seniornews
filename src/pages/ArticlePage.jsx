...
export default function ArticlePage({ user }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/articles/${encodeURIComponent(slug)}`).then(async (response) => {
      ...
      return fetchPublishedArticles({ category: loaded.category, limit: 4 });
    }).then((items) => setRelated(items.filter((item) => item.slug !== slug).slice(0, 3))).catch(() => setArticle(null)).finally(() => setLoading(false));
  ...
  return <div className="article-shell"><article className="article-wrap"><header className="article-header"><Link className="eyebrow" to={`/category/${article.category}`}>{categoryName(article.category)}</Link><h1>{article.title}</h1><p className="article-deck">{article.summary}</p><div className="meta"><strong>{article.author}</strong><time>입력 {article.publishedAt}</time>{article.updatedAt && <time>수정 {article.updatedAt}</time>}</div></header><div className="article-tools"><button className="tool-button" onClick={toggleBookmark}><Bookmark size={19} fill={bookmarked ? 'currentColor' : 'none'} /> {bookmarked ? '저장됨' : '기사 저장'}</button>{speechSupported && <button className="tool-button" onClick={toggleSpeech}>{speaking ? <Square size={18} /> : <Volume2 size={20} />} {speaking ? '읽기 멈춤' : '기사 읽어주기'}</button>}<button className="tool-button" onClick={share}><Share2 size={19} /> 공유하기</button></div>{article.image && <img className="hero-image" src={article.image} alt={article.imageAlt} />}<div className="article-body">{article.body.map((block, index) => <BodyBlock key={block.id || index} block={block} />)}{['health', 'life-finance'].includes(article.category) && <p><strong>※ 이 기사는 일반적인 정보 제공을 목적으로 하며 개인의 의료 진단이나 금융 자문을 대신하지 않습니다.</strong></p>}</div><div className="article-source"><p><strong>{article.author}</strong> | 공용 이메일 contact@mediproper.com</p><p>저작권자 © 시니어 라이프 뉴스 무단전재 및 재배포, AI학습 및 활용 금지</p></div>{related.length > 0 && <section className="related"><div className="section-heading"><h2>함께 읽으면 좋은 기사</h2></div><div className="news-grid">{related.map((item) => <ArticleCard key={item.id} article={item} />)}</div></section>}</article><NewsSidebar excludeId={article.id} /></div>;
}
