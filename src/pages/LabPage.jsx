...
function LabNav() {
  return <nav className="lab-nav" aria-label="시니어 라이프 연구소 메뉴">
    <NavLink to="/lab" end>연구소 홈</NavLink><NavLink to="/lab/about">연구소 소개</NavLink><NavLink to="/lab/research">연구 보고서</NavLink><NavLink to="/lab/insight">인사이트</NavLink><NavLink to="/lab/contact">협력 문의</NavLink>
  </nav>;
}
...
function LabHome() {
  ...
  return <>
    ...
    <section className="lab-section lab-research-feature"><div className="lab-section-heading"><div><p className="lab-kicker">LATEST RESEARCH</p><h2>최신 연구·리포트</h2></div><Link to="/lab/research">연구 보고서 <ArrowRight size={16} /></Link></div><div className="lab-research-grid">{researchItems.map((item) => <ResearchCard item={item} key={item.slug} />)}</div></section>
