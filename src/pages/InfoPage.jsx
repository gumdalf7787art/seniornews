const content = {
  about: { title: '시니어 라이프 뉴스 소개', body: <><p>시니어 라이프 뉴스는 50~70대 독자가 일상에서 바로 활용할 수 있는 건강, 복지, 생활, 일자리, 디지털, 문화 정보를 쉽고 정확하게 전하는 온라인 매체입니다.</p><h2>우리가 지키는 원칙</h2><p>어려운 용어는 풀어 쓰고, 공식 자료와 취재 출처를 확인하며, 독자가 중요한 결정을 내리는 데 필요한 맥락을 함께 제공합니다.</p></> },
  ...
  terms: { title: '이용약관', body: <><p>시니어 라이프 뉴스의 콘텐츠는 정보 제공을 목적으로 합니다. 의료·금융 관련 기사는 개인을 위한 진단이나 자문을 대신하지 않습니다.</p><p>기사의 무단 복제와 재배포는 제한되며, 인용 시 출처와 원문 주소를 표시해야 합니다.</p></> },
...
export default function InfoPage({ type }) { const page = content[type] || content.about; return <article className="info-page"><span className="eyebrow">시니어 라이프 뉴스 안내</span><h1>{page.title}</h1>{page.body}</article>; }
